...
def make_track(popularity:int, id:str, track_name:str, artist_name:str, year:str) -> Dict:
    return {
...
def songs_with_audio_features(credentials_path: str) -> None:
    ...
    df_with_songs['genre'] = df_with_songs['genre'].str.replace("'", "")
    df_with_songs['genre'] = df_with_songs['genre'].str.replace("[", "").str.replace("]", "")
    df_with_songs['genre'] = df_with_songs['genre'].str.strip(' ').str.split(',')
